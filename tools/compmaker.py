...
LATTERS_TO_ADD = frozenset(("setur",))
...
class Fixer:
    ...
    def former(self, y):
        ...
        # Nafnorð
        if (
            y[2] in {"kk", "kvk", "hk"} and not "gr" in y[5]
        ):  # Fann nafnorð, mark er í y[5]
            if "EF" in y[5]:
            ...
            if y[2] == "kvk" and "ÞFET" in y[5]:  # Stofn kvenkynsorða
                if y[0].endswith(
                    ("un", "an", "ing", "ung", "öll")
                ):  # Ath. truflar 'baun', 'laun'...
                    return
                if self.ends_in_consonant(y[0]):
                    if (
                        "ö" in y[0][-3:]
                    ):  # Orð eins og tönn, sök, dvöl, höfn... samsetningar nota 'a' í staðinn
                        oindex = y[0].rfind("ö")
                        subbed = y[0][:oindex] + "a" + y[0][oindex + 1 :]
                        self.former_parts.add(subbed)
                    else:
                        self.former_parts.add(
                            y[4]
                        )  # Orð sem enda á samhljóða, eins og urð, ... Stofninn er nefnimyndin.
                elif y[4].endswith("i"):
                    ...
                    self.former_parts.add(
                        y[4] + "s"
                    )  # Stór hluti þessara orða leyfir bandstaf, sbr. hæfnismat, keppnisskap, ...
                # elif y[0].endswith("a"):
                ...
                # pass                                # Orð eins og langnefja, ... Stofninn er ekki notaður sem fyrri hluti.
                    ...
                    self.former_parts.add(
                        y[0]
                    )  # Orð eins og skrá, þrá, ró, trú, ... Stofninn er nefnimyndin.
            elif y[2] == "hk" and "NFET" in y[5]:  # Stofn hvorugkynsorða
                if y[0].endswith("land"):
                    return
                self.former_parts.add(
                    y[4]
                )  # Orð eins og hús, rán, haf; firma, nammi, vé, sjampó, ... stofninn er nefnimyndin
                ...
                if y[0].endswith("ur"):
                    self.former_parts.add(
                        y[4]
                    )  # Orð eins og hestur, fögnuður, maður, vegur, ... stofninn er þolfallsmyndin.
                elif y[0].endswith("i"):
                    # self.former_parts.add(y[0][:-1])        # Orð eins og gróði, fjöldi, kappi, ... stofninn er án sérhljóðans. Virðast ekki notuð í samsetningum!
                    pass
                elif (
                    "ö" in y[0][-5:]
                ):  # Orð eins og vörður, völlur, köstur, ... samsetningar nota 'a' í staðinn
                    subbed = y[0][: y[0].rfind("ö")] + "a" + y[0][y[0].rfind("ö") + 1]
                    self.former_parts.add(subbed)
                else:
                    self.former_parts.add(
                        y[4]
                    )  # Orð eins og þistill, bróðir, skór, steinn, jökull, aftann, karl, víðir, herra, ... stofn er þolfallsmyndin.
        ...
        # Lýsingarorð
            ...
            if "FSB-KVK-NFET" in y[5]:
                if y[0].endswith("ur"):
                    if y[2].endswith(
                        "ur"
                    ):  # Stofninn inniheldur -ur: fagur, napur, dapur, ... Stofninn er nf.et.kk.
                    ...
                    "all"
                ):  # Orð eins og gamall, einsamall, ... Stofninn er nf.et.kk. -l.
                    self.former_parts.add(y[0][:-1])
                else:  # Orð eins og grænn, laus, alger, grár, vansvefta, ... Stofninn er nf.et.kvk.
                    if (
                        "ö" in y[0][-3:]
                    ):  # Orð eins og jöfn, sönn, gjörn, ... samsetningar nota 'a' í staðinn
                        subbed = (
                            y[0][: y[0].rfind("ö")] + "a" + y[0][y[0].rfind("ö") + 1]
                        )
                        self.former_parts.add(subbed)
                    else:
                        self.former_parts.add(y[4])
    ...
    def latter(self, y):
        ...
        if y[2] == "ao" and y[3] == "ob":
            return
        if (
            y[1] in FORBIDDEN_IDS
            or y[4] in OUTLATTERS
            or "GM-BH-ST" in y[5]
            or "MM-SAGNB" in y[5]
        ):
            return
            ...
            with open(resources_file(filename), "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if len(y) < 6:
                        ...
                        continue
                    if (
                        y[2] in FORBIDDEN_CLASSES
                        or " " in y[0]
                        or (set(y[4]) & ILLEGAL_CHARS)
                    ):
                        continue
                    self.former(y)
