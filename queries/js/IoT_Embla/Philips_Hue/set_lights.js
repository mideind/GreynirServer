...
const STATE_EX = JSON.stringify({bri_inc: 64})
...
async function getIdObject(query) {
    ...
    for (let light in allLights) {
        if (allLights[light].name === query) {
            return_object = {id: light, type: "light", url: `lights/${light}/state`}
            return return_object
        }
    }
    for (let group in allGroups) {
        if (allGroups[group].name === query) {
            return_object = {id: group, type: "group", url: `groups/${group}/action`}
            return return_object
        }
    }
    //vantar e.k. error
...
async function setLights(query, state){
    ...
    if (parsed_state.scene) {
        ...
        console.log("New brightness = " + (parsed_state.bri_inc + parsed_state.bri))
        state = JSON.stringify(parsed_state)
    }
    // Send data to API
